...
func (e *newAPIPolicySubagentEvidence) add(value string) {
	raw := strings.TrimSpace(value)
	if raw == "" {
		return
	}
...
func resolveNewAPIPolicyRootSessionID(c *gin.Context, info *relaycommon.RelayInfo, currentSessionID string) (string, string) {
	...
	if metadataRoot != "" {
		...
		if leafID == "" || !validNewAPIPolicyRootSessionUUID(metadataRoot) || !validNewAPIPolicyRootSessionUUID(leafID) || (parentID != "" && !validNewAPIPolicyRootSessionUUID(parentID)) {
			return "", newAPIPolicyRootSessionConflict
		}
		childGraph := leafID != metadataRoot
		corroboratedChild := parentID != "" || evidence.subagents.present
	...
	if headerSession != "" {
		...
		nativeGraph := leafID != "" || parentID != "" || evidence.subagents.present
		...
		if nativeGraph {
			...
			if !validNewAPIPolicyRootSessionUUID(headerSession) || !validNewAPIPolicyRootSessionUUID(leafID) || (parentID != "" && !validNewAPIPolicyRootSessionUUID(parentID)) {
				return "", newAPIPolicyRootSessionConflict
			}
			if headerSession != leafID && parentID == "" && !evidence.subagents.present {
				return "", newAPIPolicyRootSessionConflict
			}
		}
	...
	if leafID != "" || parentID != "" || evidence.subagents.present {
		return "", newAPIPolicyRootSessionUnavailable
	}
	if currentSessionID == "" {
		return "", newAPIPolicyRootSessionUnavailable
...
func (e *newAPIPolicyRootSessionEvidence) conflict() bool {
	return e == nil || e.metadataBroken || e.subagents.broken || e.headerSessions.conflict || e.parents.conflict || e.metadataRoots.conflict || e.threads.conflict || e.clientRequests.conflict || e.windows.conflict
}
...
func (e *newAPIPolicyRootSessionEvidence) addTurnMetadata(raw []byte, present bool) {
	...
	e.subagents.add(metadata.SubagentKind)
	e.subagents.add(metadata.OpenAISubagent)
	e.subagents.add(metadata.UnderscoreOpenAISubagent)
}
