...
sever 的 codex2api 档位回传：绑定目标和 API Key 必须匹配策略配置，而且本次出站请求必须带有匹配请求、渠道和用户 ID 的内部签名上下文。仅接受 HTTP 200 的有效 JSON 或成功/不完整终态 Responses SSE；失败事件、任意工具文本、未知版本和不一致元数据均不采纳。观察在协议转换前进行，不依赖模型日志开关、不改写客户端响应，重试时重新创建观察状态。单事件或 JSON 超过 2 MiB 时跳过回传观察，仍保留原始请求计费。
