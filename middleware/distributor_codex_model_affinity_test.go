...
func TestCodexModelSwitchKeepsChannelBindingAndRequiresNewConversation(test *testing.T) {
	...
	require.Contains(test, recorder.Body.String(), "请选择其他可用模型继续当前任务")
	require.Contains(test, recorder.Body.String(), "请新建对话后重试")
	require.NotContains(test, recorder.Body.String(), "上游渠道")
